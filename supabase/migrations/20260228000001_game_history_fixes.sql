-- Migration 009b: Fix record_game_result comments and get_game_history LEFT JOIN.
--
-- Changes vs 009:
--   1. record_game_result: added comment on both IF p_game_id IS NOT NULL blocks
--      explaining the silent zero-row UPDATE for vs-AI games is intentional.
--   2. get_game_history: changed INNER JOIN on game_players opp to LEFT JOIN so
--      vs-AI games (which have no AI row in game_players) appear in history.
--      The downstream COALESCE(u.display_name, 'AI') already handles NULL opponent fields.
