...
describe('expectimaxPeggingPlay', () => {
  it('should return a number (EV between 0 and 31)', () => {
    ...
    const ev = expectimaxPeggingPlay(gameState, 5, 1, 12345);
  ...
  it('should be deterministic with same seed', () => {
    const gameState = createGame(2);
    const ev1 = expectimaxPeggingPlay(gameState, 5, 2, 12345);
    const ev2 = expectimaxPeggingPlay(gameState, 5, 2, 12345);
    expect(ev1).toBe(ev2);
  });
  ...
  it('should run without error for different seeds', () => {
    ...
    const ev1 = expectimaxPeggingPlay(gameState, 10, 2, 1);
    const ev2 = expectimaxPeggingPlay(gameState, 10, 2, 999);
